        ...
        >
          <template #dropdownRender="{ menuNode: menu }">
            ...
            >
              ...
              >
                <a-input
                  v-model:value="newBecomeMethod"
                  placeholder="输入新的特权方法"
                ></a-input>
              </a-modal>
            </div>
          </template>
          <a-select-option v-for="b in becomeMethods" :key="b">{{
            b
          }}</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item label="特权用户" name="becomeUser"
        ><a-input v-model:value="form.becomeUser"
      /></a-form-item>
      <a-form-item label="特权密码" name="becomePassword"
        ><a-input-password v-model:value="form.becomePassword"
      /></a-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        username: [
          { required: true, message: "请输入凭证的用户名。", trigger: "blur" }
        ],
        password: [
          {
            required: true,
            message: "请输入与凭证用户名对应的密码。",
            trigger: "blur"
          }
        ],
        becomeUser: [
          {
            message: "请输入与特权方法对应的密码。",
            trigger: "blur",
        ...
        description: [
          { max: 512, min: 0, message: "描述信息长度不应该超过 512" }
        ]
      }
    };
  },
